...
class FP_solver():
    ...
    def __init__(self, k, rho, Cp, H, A, E, kinetics_model, T0, alpha0, x_gs, x_ge, hl, T_trig=None, t_trig=1, L=0.03, dt=1e-3, h_x=1e-5):
        ...
        x_gs : float
            Location of beginning of gap. Always 0
        x_ge : float
            Location of end of gap
        hl : float
            heatloss coefficient value
        T_trig : float
            The trigger temperature in deg. C. If no value supplied, uses T_max of adiabatic system
        t_trig : float
            Duration of tTrigger
        ...
        # max temperature of reaction in adiabatic system
        self.T_max = self.T0 + H/Cp*(1-alpha0)
        if not T_trig:
        ...
        self.L = L
        self.h_x = h_x
        self.nel_x = math.ceil((L+x_ge)/h_x)
    ...
    def setSystemParameters(deg):
        ...
        :param deg: Degree of quadrature
        :type deg: int
    ...
    def local_project(v, V, u):
        ...
        :param v: A ufl algebraic expression referencing the data to be projected
        :type v: ufl operator

        :param V: The function space to which function u belongs
        :type V: dolfin function space

        :param u: The function onto which values are projected
    ...
    def setupSolver(self):
        ...
        # Set leftbounds and rightbounds as -domain/2, domain/2 + gap
        gapsize = self.x_ge
        leftbound = 0 - (self.L / 2)
        rightbound = (self.L / 2) + gapsize
        ...
        left.mark(facets, 1)
        right.mark(facets, 2)
        # partition boundary into left and right
        ds = Measure('ds', domain=mesh, subdomain_data=facets)

        # Create function spaces for temperature and degree of cure fields
        # continuous 1st order polynomials for temperature
        V = FunctionSpace(mesh, "CG", 1)

        dx = Measure('dx', domain=mesh, metadata={
                     "quadrature_degree": self.QUAD_DEG, "quadrature_scheme": "default"})
        Qelement = FiniteElement("Quadrature", mesh.ufl_cell(
        ), degree=self.QUAD_DEG, quad_scheme="default")
        # define degree of cure at quadrature points
        ...
        T = TrialFunction(V)
        # function storing values of temperature at current time
        T_ = Function(V, name="Temperature")
        # function storing values of temperature at previous time
        T_n = Function(V)
        # function tracking maximum temperature in domain over time
        T_max = Function(V, name="T_max")
        # function storing values of degree of cure at current time
        alpha_ = Function(Q)
        # function storing values of degree of cure at previous time
        alpha_n = Function(Q)
        # function used for displaying degree of cure in results file
        alpha_plot = Function(V, name="alpha")
        # Y2 = FunctionSpace(mesh, "DG", 0)
        # alpha_plot = Function(Y2, name="alpha")

        # Define boundary conditions
        T_trig_val = Constant(self.T_trig)
        # dirichlet boundary condition on left end of domain
        bc_trig = DirichletBC(V, T_trig_val, left)
        bcs_trig = [bc_trig]  # boundary conditions with thermal trigger
        # boundary conditions after thermal trigger removed (adiabatic)
        bcs_adiab = []

        # Piecewise expression for the gap region. Setting A=0 in gap ensures no reactivity. Heatloss only in gap or everywhere?
        ...
        problem_T_trig = LinearVariationalProblem(lhs(F_T), rhs(
            F_T), T_, bcs=bcs_trig, form_compiler_parameters=self.ffc_options)
        ...
        problem_T_adiab = LinearVariationalProblem(lhs(F_T), rhs(
            F_T), T_, bcs=bcs_adiab, form_compiler_parameters=self.ffc_options)
    ...
    def solve(self, t_end, outputFreq_t=1, outputFreq_x=1):
        ...
        :param t_end: The time span of the simulation
        :type t_end: float

        :param output_t: Frequency in time steps at which to save data efault value of 1, ie. save at every time step)
        :type output_t: int

        :param output_x: Spacing between sampling nodal points at which to save data (default value of 1, ie. save every grid point)
        :type output_x: int        
        ...
        :rtype: Arrays of float
        """
        self.t_end = t_end
        self.outputFreq_t = outputFreq_t
        self.outputFreq_x = outputFreq_x
        t = 0                                           # current time
        step = 1                                        # time step counter
        # counter for saving at sampled time points
        t_idx = 1
        # boolean for whether simulation is complete
        solve_complete = False
        # number of time steps in simulation
        num_steps = math.ceil(t_end / self.delta_t)

        # Initialize x coordinate and time vectors at sampled points
        # indices of dof at which to save data
        save_x_idx = np.arange(0, self.nel_x, outputFreq_x)
        # ensure end point is included in sampling
        save_x_idx[-1] = self.nel_x
        coords = self.T_.function_space().tabulate_dof_coordinates()
        # get the sorted indices for dof in positive x direction
        coords_sort_idx = coords.reshape(-1).argsort()
        # vector containing the x coordinates at sampled points
        x_coords = coords[coords_sort_idx][save_x_idx]
        # vector containing the t values at sampled times
        t_arr = np.linspace(0, t_end, math.ceil(num_steps/outputFreq_t)+1)
        ...
        Tpeak = self.T0*np.ones(x_coords.shape[0])
        # dof index beyond which front not recorded (does not save once front becomes close to end of domain)
        idx_end = math.ceil(0.9*self.nel_x + 1)
        ...
        while not solve_complete and step < num_steps+1:
            ...
            self.local_project(self.alpha_update,
                               self.alpha_.function_space(), self.alpha_)
            self.alpha_.vector()[:] = np.minimum(
                self.alpha_.vector()[:], 1-1e-4)
            self.alpha_.vector()[:] = np.maximum(
                self.alpha_.vector()[:], self.alpha0-1e-4)
            ...
            # get T_peak
            Tpeak = np.maximum(self.T_.vector()[coords_sort_idx][save_x_idx], Tpeak)
            ...
            if step % outputFreq_t == 0:
                ...
                self.printParallel(f"Saving results for t = %f s, Elapsed simulation time: %f s" % (
                    t, time.time() - start_time))

                # Project alpha from quadrature points to nodes
                self.local_project(
                    self.alpha_, self.alpha_plot.function_space(), self.alpha_plot)
                self.alpha_plot.vector()[:] = np.minimum(
                    self.alpha_plot.vector()[:], 1-1e-4)
                self.alpha_plot.vector()[:] = np.maximum(
                    self.alpha_plot.vector()[:], self.alpha0-1e-4)

                # sort alpha values along positive x
                alpha_sorted = self.alpha_plot.vector()[coords_sort_idx]
                # Save temperature and degree of cure
                if alpha_sorted[idx_end] < 0.5:
                    T_data[t_idx, :] = self.T_.vector(
                    )[coords_sort_idx][save_x_idx]
        ...
        results_data = {'x_data': x_data, 't_data': t_data,
                        'alpha_data': alpha_data, 'T_data': T_data, 'T_peak': Tpeak}
